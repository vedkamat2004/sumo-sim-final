...
class target_vehicles_generator:
    ...
    def generate_vehicles(self, num_target_vehicles, num_random_vehicles, pattern, target_xml_file, net_xml_file):
        ...
        try:
            ...
            randomTrips_source = os.path.join(sumo_tools, 'randomTrips.py')
            randomTrips_dest = './randomTrips.py'
            shutil.copy2(randomTrips_source, randomTrips_dest)
        except Exception as e:
            print(f"ERROR: Failed to copy randomTrips.py to current directory: {e}")
            return None
        #invoke randomTrips.py
        try:
            result = subprocess.run([sys.executable, 'randomTrips.py', '-n', net_xml_file, '-e', '50', '-p', str(density), '-r', target_xml_file], 
                                    capture_output=True, text=True)
            if result.returncode != 0:
                print(f"ERROR: Failed to invoke randomTrips.py: {result.stderr}")
                return None
        except Exception as e:
            print(f"ERROR: Failed to invoke randomTrips.py: {e}")
            return None
        #delete randomTrips.py
        try:
            os.remove('./randomTrips.py')
        except Exception as e:
            print(f"ERROR: Failed to remove randomTrips.py: {e}")
            return None
