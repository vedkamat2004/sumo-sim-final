**_Selfless Traffic Routing testbed based on SUMO (STR-SUMO)_**
...
It will show the benchmarking results of the A\* routing policy for a set of vehicles sharing the same start point and the same destination.
...
- AStarController.py: the routing plicy that employs A\* to find the shortest path (without considering congestion) for each controlled vehicle;
